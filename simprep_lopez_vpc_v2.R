...
# Calculate 5, 50 and 95 percentiles for each simulated study (S)
  ...
  p2 <- p2 + stat_summary(aes(x = TIMEBIN, y = loCI90S, colour = Model),
    fun.y = median, geom = "line", linetype = "dashed", size = 0.8, alpha = 0.7)
  ...
  p2 <- p2 + scale_y_log10("Concentration (ng/mL)\n",
  ...
  ggsave("Uppsala_VPC_v2.png", width = 17.4, height = 14, units = c("cm"))
  ggsave("Uppsala_VPC_v2.eps", width = 17.4, height = 14, units = c("cm"),
