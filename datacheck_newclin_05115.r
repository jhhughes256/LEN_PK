...
#  Organise working and output directories
  working.dir <- paste0(master.dir, "/RAW_Clinical/rawdata-lena_05115/")
...
# # #  Updated from datacheck_front.r 			# reproducible
  file.name.in <- paste0(working.dir, "rawdata-lena_CR.csv")
  datanew <- read.csv(file.name.in, stringsAsFactors = F, na.strings = c("."))

  # #  Wt_Ht_BMI.xlsx - 13 sheets of data
  file.name.in2 <- paste0(working.dir, "OSU05115_06003_Wt_Ht_BMI_n63.xlsx")
  demog.ht <- read_excel(file.name.in2, sheet = 1)  # demographics

  # # corr_analysis.xls - 9 sheets of data
  file.name.in3 <- paste0(working.dir, "Copy of Correlation analysis-Non compartment.xls")
  datapd.in <- data.frame(read_excel(file.name.in3, sheet = 9)[-1, -(2:24)])  # rawdata
