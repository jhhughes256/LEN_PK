...
#-------------------------------------------------------------------------------------
   ...
   dataallone <- lapplyBy(~UID, data=ivdfall,  oneperID)
   ...
   dataallone$UIDf <- as.factor(dataallone$UID)
   dataallone$IDf <- as.factor(dataallone$ID)
   dataallone$DOSEMGKGf <- as.factor(dataallone$DOSEMGKG)
   
   dataallone2 <- melt(dataallone, id=c("UID","ID","TADNOM","DOSEMGKG","DOSEMG","AMT","WT","TIME","UIDf","IDf","DOSEMGKGf"))
   colnames(dataallone2) <- c(head(colnames(dataallone),11),"TISSUE","DV")
...
#-------------------------------------------------------------------------------------
   ...
   #Keeps missing as -1 - use for categorical summary
   dataallone <- lapplyBy(~UID, data=podfall,  oneperID)
   dataallone <- bind.list(dataallone)
   dim(dataallone)
...
#-------------------------------------------------------------------------------------
   ...
   #Keeps missing as -1 - use for categorical summary
   dataallone <- lapplyBy(~UID, data=prdfall,  oneperID)
   dataallone <- bind.list(dataallone)
   dim(dataallone)
