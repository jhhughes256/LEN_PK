...
# Define the model parameters and equations
  ...
  # This compiled model is used for simulating n individuals and their concentration-time profiles
    ...
    $PARAM    // Population parameters
              ...
              POPKTR = 0.5,  // Absorption rate constant, h^-1

              // Covariate effects
              COV1 = 0.5,  // Effect of creatinine clearance on clearance

              // Default covariate values for simulation
              WT = 70,  // Total body weight (kg)
							HT = ,  // Patient height (cm)
              CRCL = 0,  // Gender - Male (1), Female (0)
              AGE = 60,  // Age (years)
    ...
    $OMEGA    block = TRUE
              ...
              0.0000 0.0000 0.0256  // BSV for KA
    ...
    $MAIN      // Covariate effects
              ...
              if (SEX == 0) CRCL = ((140-AGE)*WT)/(SECR*0.815)*0.85;  // Female creatinine clearance

              // Individual parameter values
              double CL = POPCL*pow(FFM/70,0.75)*exp(BSV_CL);
              double V1 = POPV1*(FFM/70)*exp(BSV_V1);
              double KA = POPKTR*exp(BSV_KTR);

    $ODE      // Differential equations
              dxdt_DEPOT = -KTR*DEPOT;
							dxdt_TRC1 = KTR*DEPOT -KTR*TRC1
							dxdt_TRC2 = KTR*TRC1 -KTR*TRC2
							dxdt_TRC3 = KTR*TRC2 -KTR*TRC3
							dxdt_TRC4 = KTR*TRC3 -KTR*TRC4
							dxdt_TRC5 = KTR*TRC4 -KTR*TRC5
							dxdt_TRC6 = KTR*TRC5 -KTR*TRC6
							dxdt_TRC7 = KTR*TRC6 -KTR*TRC7
              dxdt_CENT = KTR*TRC7 -CL/V1*CENT;
              dxdt_AUC = CENT/V1;

    $TABLE    table(IPRE) = CENT/V1;
              table(DV) = table(IPRE)*(1+ERR_PRO);

    $CAPTURE  CL V1 KA FFM CRCL AUC
    '
